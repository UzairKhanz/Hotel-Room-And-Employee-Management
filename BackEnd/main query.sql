...
END //
DELIMITER ;
CALL add_login('uzair',1234);

DELETE FROM login where user_id='uzair';
...
DELIMITER //
CREATE PROCEDURE CalculateTotalProfit(
    OUT totalValue INT,
    OUT totalSaleValue INT,
    OUT totalProfit INT
)
BEGIN
    -- Calculate total value
    SELECT SUM(quantity * buy_price) INTO totalValue
    FROM Product
    JOIN Inventory ON Product.product_id = Inventory.product_id;

    -- Calculate total sale value
    SELECT SUM(quantity * sell_price) INTO totalSaleValue
    FROM Product
    JOIN Inventory ON Product.product_id = Inventory.product_id;

    -- Calculate total profit
    SET totalProfit = totalSaleValue - totalValue;
END //
DELIMITER ;

DROP procedure CalculateTotalProfit;

CALL CalculateTotalProfit(@totalValue, @totalSaleValue, @totalProfit);
SELECT @totalValue AS TotalValue, @totalSaleValue AS TotalSaleValue, @totalProfit AS TotalProfit;

DROP DATABASE stationery;
